...
[^1]: 患者病历、付费项目信息、疾病信息等，类似于数据库；这类数据，暂时以类的方式解析，之后再转换成直接解析文件内容。

[^2]: resource 文件夹充当了数据库的作用。
...
[^3]: Physical examinations, chemical tests, medications and treatments must be paid in order for patients to get them. Before these items are actually used, the patient may ask for a refund. A refund for medications, particularly, is performed after they are returned back to the pharmacy.